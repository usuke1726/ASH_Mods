...
internal class ItemWrapperBase
{
    ...
    private string GetDefaultLocalizedName() => I18nLocalize(false, $"item.{id}.name");
    private string GetDefaultLocalizedNamePlural() => I18nLocalize(false, $"item.{id}.namePlural");
    internal void OnLocaleChanged()
    {
        if (!State.IsActive) return;
        var i18nKeys = GetI18nKeys();
        string s;
        s = I18nLocalize(false, i18nKeys.ReadableName);
        if (string.IsNullOrEmpty(s)) s = GetDefaultLocalizedName();
        item.readableName = string.IsNullOrEmpty(s) ? defaultReadableName : s;
        s = I18nLocalize(false, i18nKeys.ReadableNamePlural);
        ...
        s = I18nLocalize(false, i18nKeys.Description);
